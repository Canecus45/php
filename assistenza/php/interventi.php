...
<div class ="container">
        ...
        $matricola = $_REQUEST['matricola']; //è indifferente uso $_REQUEST o $_GET
        //Elenco tutti gli interventi  effettuati dal tecnico selezionato
        //DATE_FORMAT è una funzione del linguaggio SQL che consente di formattare una data nel formato gg//mm/aaaa
        $query="SELECT numero, TICKET, TECNICO, DATE_FORMAT(data, '%d/%m/%Y') as data FROM interventi WHERE TECNICO = $matricola";
        
        $tabella=$conn->query($query);
        if ($tabella->num_rows == 0)
            die("Non ci sono interventi effettuati dal tecnico selezionato");
        ?>
