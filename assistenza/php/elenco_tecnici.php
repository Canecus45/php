...
<body>
    ...
    <div class ="container">
        ...
        if ($tabella->num_rows == 0)
            ...
            echo "<table class='table table-bordered'> <tr><td>Matricola</td><td>Cognome</td><td>Nome</td></tr>";
            ...
            while($row=$tabella->fetch_assoc()){
                ...
                //l'uso del tag href ci permetterà di rendere cliccabile la matricola e passarla come parametro alla pagina interventi.php
                echo "<tr><td><a href=interventi.php?matricola={$row['matricola']}>{$row['matricola']}</a></td><td>{$row['cognome']}</td><td>{$row['nome']}</td></tr>";
            }
            echo "</table>";
        }
        ?>
